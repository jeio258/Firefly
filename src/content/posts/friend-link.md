...
# 流萤 / Firefly —— 一款清新美观的 Astro 静态博客主题模板

**流萤 (Firefly)** 是一款基于 Astro 框架构建的现代化个人博客主题，继承了 Fuwari 的优秀基因，并在此基础上大幅丰富了布局与交互。它拥有左右双侧边栏、多列文章布局、瀑布流、音乐播放器、分享海报等众多组件，同时保留了原版布局选项，让你自由打造独一无二的博客。

- 演示站点：[在线预览](https://firefly.cuteleaf.cn/)
- 使用文档：[文档](https://docs-firefly.cuteleaf.cn/)

---

## ✨ 功能速览

- **现代技术栈**：Astro + Tailwind CSS，极快的页面加载与天然 SEO 优化。
- **流畅动画**：基于 Swup 的页面过渡，体验丝滑。
- **响应式布局**：针对桌面、平板、手机全面优化。
- **多语言 UI**：内置简体中文、繁体中文、英文、日文、俄语、韩文。
- **全文搜索**：基于 Pagefind 的客户端搜索，支持文章内容索引。
- **友链自助申请与后台审核**：访客可自助提交友链，管理员通过 GitHub OAuth 登录后台审核，数据自动更新到仓库。
- **丰富的个性化选项**：主题色、壁纸模式、字体、导航栏、页脚、侧边栏数量、文章排列方式等均可配置。
- **Markdown 扩展**：提醒块、GitHub 仓库卡片、增强代码块等开箱即用。
